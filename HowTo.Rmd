...
In this document we show how to use the data available on this site and especialy annotations in order to do sentiment analysis on the COVD19-TweetIDs that are being published on this site (french language tweets).

## Downloading and preparing files for analysis
This preparation phase uses essentially system bash commands

### Clone this COVD19-TweetIDs-FR github repository on a local computer
First, in order to accelerate calculations, it is recomandable to clone this site on your local computer. Here we select the local user's Downloads directory in order to place a copy of this github repository.

```{bash}
cd ~/Downloads/
git clone https://github.com/calciu/COVID19-TweetIDs-Fr
```
...
```{bash}
cd ~/Downloads/COVID19-TweetIDs-Fr/CovidAnnot
...
Removing the underscore in the name of the annotation file helps distinguish annotation from helper files.

```{bash}
cd ~/Downloads/COVID19-TweetIDs-Fr/CovidAnnot
cat nrc1-5fr.csv > _nrc1-11fr.csv && tail -n+2 -q nrc??fr.csv >> _nrc1-11fr.csv
rm n*
mv _nrc1-11fr.csv  nrc1-11fr.csv # remove the underscore in front of the annotation file as a distinction from helper
```
...
```{r}
setwd("~/Downloads/COVID19-TweetIDs-Fr/CovidAnnot")
